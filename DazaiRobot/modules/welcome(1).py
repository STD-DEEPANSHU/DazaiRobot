...
markup=InlineKeyboardMarkup ([[InlineKeyboardButton ("MODS", url="https://t.me/StdBotz")]])
...
"""
        ...
        try:
            ...
            for x in a:
                ...
                resp = r.post("https://shrouded-thicket-70974-7801cc6fa987.herokuapp.com/generate", json={'name' : f'{u.first_name}','user_name' : f'@{u.username}','user_id' : f'{u.id}','profile_pic' : f'{url}','group_name' : f'{message.chat.title}' ,"auth_key" : "Yash_Yash__@"}).json()
                await message.reply_photo((resp['image_url']), caption=MSG, reply_markup=markup)
                os.remove(f"./downloads/pfp_{uid}.png")
        except AttributeError:
            resp = r.post("https://shrouded-thicket-70974-7801cc6fa987.herokuapp.com/generate", json={'name' : f'{u.first_name}','user_name' : f'@{u.username}','user_id' : f'{u.id}','group_name' : f'{message.chat.title}' ,"auth_key" : "Yash_Yash__@"}).json()
            await message.reply_photo((resp['image_url']), caption=MSG, reply_markup=markup)
            os.remove(f"./downloads/pfp_{uid}.png")
